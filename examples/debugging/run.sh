...
ssm run -p ${2}ssm-debugging -t metrics${3} -e ./metrics.py -o $OUTPUT/output1 ${@:4} \
    --no_spot `#temporarily to accelerate iterations` &

ssm run -p ${2}ssm-debugging -t tensorboard${3} -s ./tensorboard -e lightning.py -o $OUTPUT/output2 ${@:4} \
