...
ssm run -p ${2}ssm-debugging${3} -t metrics -e ./metrics.py -o $OUTPUT/output1 ${@:4} \
    --no_spot `#temporarily to accelerate iterations` &

ssm run -p ${2}ssm-debugging${3} -t tensorboard -s ./tensorboard -e lightning.py -o $OUTPUT/output2 ${@:4} \
